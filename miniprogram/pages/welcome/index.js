...
Page({
  ...
  async createFamily() {
    ...
    try { await call('createFamily', { name: this.data.familyName.trim() }); wx.switchTab({ url: '/pages/menu/index' }) } finally { this.setData({ loading: false }) }
  },
  async joinFamily() {
    if (this.data.inviteCode.length !== 6) return wx.showToast({ title: '请输入 6 位邀请码', icon: 'none' })
    this.setData({ loading: true })
    try { await call('joinFamily', { inviteCode: this.data.inviteCode }); wx.switchTab({ url: '/pages/menu/index' }) } finally { this.setData({ loading: false }) }
  }
})
