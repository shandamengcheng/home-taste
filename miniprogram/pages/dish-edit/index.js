...
Page({
  data: { id: '', name: '', emoji: '🍲', color: colors[0], emojis: ['🍲','🥘','🍜','🍛','🥗','🍗','🐟','🥩','🥟','🍳'], ingredients: [{ name: '', amount: '' }], tagsText: '', note: '', saving: false },
  async onLoad(options) {
    if (!options.id) return
    const dish = await call('getDish', { id: options.id })
    this.setData({ id: dish._id, name: dish.name, emoji: dish.emoji, color: dish.color, ingredients: dish.ingredients.length ? dish.ingredients : [{name:'',amount:''}], tagsText: (dish.tags || []).join(', '), note: dish.note || '' })
  },
  ...
  async save() {
    ...
    try {
      await call('saveDish', { dish: { _id: this.data.id || undefined, name: this.data.name.trim(), emoji: this.data.emoji, color: this.data.color, ingredients, tags: this.data.tagsText.split(/[,，]/).map(s => s.trim()).filter(Boolean), note: this.data.note.trim() } })
